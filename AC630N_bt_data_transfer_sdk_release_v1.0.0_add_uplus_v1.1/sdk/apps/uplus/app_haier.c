...
static void app_key_event_handler(struct sys_event *event)
{
    ...
    if (event->arg == (void *)DEVICE_EVENT_FROM_KEY) {
        ...
        if (event_type == KEY_EVENT_CLICK && key_value == TCFG_ADKEY_VALUE0) {
            ...
            uplus_ble_gatts_mtu_get(0x50);
            uplus_ble_gatts_mtu_default_set(200);
        ...
        }else if(event_type == KEY_EVENT_CLICK && key_value == TCFG_ADKEY_VALUE5){
            y_printf("led off or uplus get[%d]\n",uplus_gpio_in_get(&input_pin));
            uplus_gpio_out_set(&output_pin,0);
        }else{
            y_printf("key no task!!!");
        }
