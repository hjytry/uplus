 ...
 * @LastEditTime: 2021-09-26 09:21:45
 * @LastEditors: jianye
 */
...
static u32 uplus_timer_param_table[UPLUS_TIMER_END] = {0};  
int8_t uplus_timer_create(uplus_timer_id_e id, uplus_timer_mode_e mode, uint32_t interval, uplus_timer_handler_t time_handler)
{
    ...
    if((id >UPLUS_TIMER_END) || (id <UPLUS_POLL_TIMER) || (time_handler == NULL)){
    ...
    if(uplus_timer_param_table[id] == NULL){
...
void uplus_timer_start(uplus_timer_id_e timer_id)
{
    ...
    if(uplus_timer_param_table[timer_id] == NULL){
    ...
    sys_timer_create = (ptimer->mode == UPLUS_TIMER_SINGLE_SHOT) ? sys_timeout_add : sys_timer_add;
    ptimer->timer_id = sys_timer_create(NULL,ptimer->timeout_handler,ptimer->timeout_value_ms);
