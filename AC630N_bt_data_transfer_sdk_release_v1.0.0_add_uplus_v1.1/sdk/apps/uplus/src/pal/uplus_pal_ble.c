 ...
 * @LastEditTime: 2021-10-12 13:56:37
 * @LastEditors: jianye
 */
...
/**
 * @description: 判断handle值是否为notify
 * @param  {*}
 * @return {*}
 * @Author: jianye
 * @Date: 2021-10-09 14:40:05
 * @LastEditTime: Do not edit
 * @LastEditors: jianye
 * @param {u8} svr
 * @param {u8} uchar
 * @param {uplus_ble_uuid_t} *uuid_data
 */
...
static u16 uplus_ble_gatts_char_data_init(uplus_ble_gatts_char_db_t *p_char_db,u8 *p_save_profile,u32 offset)
{
    ...
    if((p_char_db->char_property & EX_ATT_PROPERTY_NOTIFY) || (p_char_db->char_property & EX_ATT_PROPERTY_INDICATE)){
        ...
        uplus_handle++;
        p_char_db->char_handle.cccd_handle = uplus_handle;  //赋值CCCD
...
uplus_ble_status_e uplus_ble_set_adv_param(uplus_ble_adv_param_t *adv_param)
{
    ...
    if(adv_param->interval_min >= ADV_INTERVAL_MIN){
        adv_interval = adv_param->interval_min;
    }else if(adv_param->interval_max >= ADV_INTERVAL_MIN){
        adv_interval = adv_param->interval_max;
    }else{
        adv_interval = 0;
...
uplus_ble_status_e uplus_ble_gap_adv_start(uplus_ble_gap_adv_param_t *p_adv_param)
{
    ...
    if(p_adv_param->adv_interval_min >= ADV_INTERVAL_MIN){
        adv_interval = p_adv_param->adv_interval_min;
    }else if(p_adv_param->adv_interval_max >= ADV_INTERVAL_MIN){
        adv_interval = p_adv_param->adv_interval_max;
    }else{
        adv_interval = 0;
        ret  = UPLUS_BLE_ERR_INVALID_LENGTH;
        goto param_error;
...
uplus_ble_status_e uplus_ble_gap_adv_data_set(uint8_t const * p_data, uint8_t dlen,uint8_t const *p_sr_data,uint8_t srdlen)
{
    ...
    UPLUS_BLE_HEXDUMP(p_sr_data, dlen);

    ble_op_set_adv_data(dlen,p_data);
    ble_op_set_rsp_data(srdlen,p_sr_data);
...
uplus_ble_status_e uplus_ble_gatts_notify_or_indicate(uint16_t conn_handle,uint16_t srv_handle,uint16_t char_value_handle,uint8_t offset,uint8_t* p_value,int len,uint8_t type)
{
    ...
    if(UPLUS_ATT_OP_NOTIFY == type){
        app_send_user_data(char_value_handle, &p_value[offset], len, ATT_OP_NOTIFY);
    }else if(UPLUS_ATT_OP_INDICATE == type){
        app_send_user_data(char_value_handle, &p_value[offset], len, ATT_OP_INDICATE);
    }else{
 ...
 * @return {*}
 * @Author: jianye
 * @Date: 2021-10-12 10:00:50
 * @LastEditTime: Do not edit
 * @LastEditors: jianye
 * @param {uint16_t} att_mtu
 */
